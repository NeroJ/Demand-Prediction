...
class FittingPolynome:
    def __init__(self):
        self.data = xlrd.open_workbook('F2.xls')
    ...
    def MaxMinNormalization(self): 
        Y11_12_MaxMin = []
        Min = np.min(self.Y11_12)
        Max = np.max(self.Y11_12)
        for item in self.Y11_12:
            x = (item - Min) / (Max - Min)
            Y11_12_MaxMin.append(x)
        self.Y11_12_MaxMin = Y11_12_MaxMin
        return Y11_12_MaxMin
    ...
    def Fit_Origin(self):
        ...
        cof = np.polyfit(x,y,20) #for original data  range 0 ~ 50, 30 gets the biggest fitting level, with 
        p=np.poly1d(cof)
